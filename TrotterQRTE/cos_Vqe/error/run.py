...
if __name__ == "__main__":
    ...
    max_length = max(len(result) for result in result)
    nstep = [len(result_intermediate), len(result_intermediate1), len(result_intermediate2), len(result_intermediate3)]
    plot_data4('U_vqe_error', nstep, result)
    with open('U_vqe_error.txt', 'w', encoding='utf-8') as f:
       for i in range(max_length):
           row = '\t'.join('{:.12f}'.format(lst[i]) if i < len(lst) else '' for lst in result)
           f.write(f"{row}\n")
    print('Result has been stored')
