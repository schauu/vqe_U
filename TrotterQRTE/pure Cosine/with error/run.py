...
def cosine_filtering_temp(H, time, nqubits, order, time_step, error_rate, step, threshold):
    simulator = select_sim(error_rate)
    expectation_list = []
    probability_list = []
    initial_state = Statevector.from_label("0000")
    ...
    for i in range(step):
        ...
        new_state = result.data
        #postselect_state = new_state[: 2**nqubits]
        postselect_state = postselect_density.postselect_statevector(new_state,
                           nqubits+1,
                           select_qubit=nqubits,
                           select_value=0,
                           normalize= True,
                           print_debug= False)
        probability = np.linalg.norm(postselect_state)
...
if __name__ == "__main__":
    ...
    # def callback(intermediate_result):
    #     result_intermediate.append(intermediate_result.fun)
    # estimate_val = sp.optimize.minimize(cost_function, vqe_params, args=(nqubits, depth, error_rate), method="BFGS", tol=1e-5,
    #                             options={'disp': True}, callback=callback)
    # print(estimate_val)
    # result_intermediate1 = [] ## for error rate1
    # def callback1(intermediate_result):
    #     result_intermediate1.append(intermediate_result.fun)
    # estimate_val1 = sp.optimize.minimize(cost_function, vqe_params, args=(nqubits, depth, error_rate1), method="BFGS", tol=1e-5,
    #                             options={'disp': True}, callback=callback1)
    # print(estimate_val1)
    # nstep = len(result_intermediate)
    # nstep1 = len(result_intermediate1)

    #f_e, f_p = cosine_filtering(H, time, nqubits, order, time_step, error_rate, nstep, threshold)
    #
    # f_e1, f_p1 = cosine_filtering(H, time, nqubits, order, time_step, error_rate1, nstep, threshold)
    # f_e2, f_p2 = cosine_filtering(H, time, nqubits, order, time_step, error_rate2, nstep, threshold)
    ...
    # plot_data2("error_rate pure U", nstep, f_e, f_p, f_e1, f_p1, f_e2, f_p2)
    ...
    #
    # plot_data5('pure Cosine filter with Trotterization', nstep, f_e, f_p, f_e1, f_p1, f_e2, f_p2, f_e4, f_p4)
    ...
    ## Pure cosine filter with error rate
    ...
    f_e2, f_p2 = cosine_filtering(H, time, nqubits, order, time_step, error_rate2, nstep, threshold)
    plot_data2("error_rate pure U", nstep, f_e, f_p, f_e1, f_p1, f_e2, f_p2)
    with open('error_rate_pure_U.txt', 'w', encoding='utf-8') as f:
       for i in range(nstep):
           f.write(f"{f_e[i]}\t{f_e1[i]}\t{f_e2[i]}\t{f_p[i]}\t{f_p1[i]}\t{f_p2[i]}\n")
       print('Result has been stored')
